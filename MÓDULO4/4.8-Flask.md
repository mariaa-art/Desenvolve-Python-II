...
Resposta correta:  b) Associar uma URL e um método HTTP a uma função que será chamada quando tal padrão for invocado.
---
...
Resposta correta: c) @app.route('/home', methods=['POST'])
...
Resposta correta: a) Gerar páginas HTML dinâmicas com base em dados passados pela aplicação
...
Resposta correta: a) Separando a lógica de negócios (controller), a interface do usuário (view) e a modelagem de dados em componentes distintos (model)
...
Resposta correta: a) A aplicação Web está disponível no endereço IP 127.0.0.1, aceitando requisições somente do localhost.
